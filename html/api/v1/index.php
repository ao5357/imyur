...
$subdomain = (isset($_POST['subdomain']) && in_array($_POST['subdomain'], [
    'i',
    'self',
    'www',
  ])) ? $_POST['subdomain'] . '.' : '';
...
if ($rest_file == '/api/v1/shorten.html') {
  $rest_file = 'html';
}
else {
  if ($rest_file == '/api/v1/shorten.json') {
    $rest_file = 'json';
  }
  else {
    $rest_file = FALSE;
...
$scheme_good = (isset($url_parts['scheme']) && in_array($url_parts['scheme'], [
    'http',
    'https',
    'shttp',
    'ssl',
...
function save_url($input_url) {
  ...
  $sdb = new AmazonSDB();
  $response = $sdb->put_attributes('addresses', $hash, ['address' => $input_url], TRUE);
  $awssuccess = $response->isOK();
  if ($awssuccess) {
    apc_add($hash, $input_url, 0);
    return [$awssuccess, $hash];
  }
  else {
    return [$awssuccess];
  }
...
if ($scheme_good && $not_imyur) {
  ...
  if ($http_response_header[0] == 'HTTP/1.0 204 No Content' || substr($http_response_header[0], 0, 12) == 'HTTP/1.0 503') {
    ...
    if (substr($http_response_header[0], 0, 12) == 'HTTP/1.0 200') {
...
if ($success && $rest_file == 'html') {
  ...
  if ($success && $rest_file == 'json') {
    header("Content-Type: application/json; charset=UTF-8");
    echo json_encode($output);
  }
  else {
    if (!$success && isset($output['error']) && $rest_file == 'html') {
      header("HTTP/1.0 400 Bad Request");
      echo 'There was a problem with your request.<br /><strong>Error output</strong>: ' . $output['error'];
    }
    else {
      if (!$success && isset($output['error']) && $rest_file == 'json') {
        header("HTTP/1.0 400 Bad Request");
        header("Content-Type: application/json; charset=UTF-8");
        echo json_encode($output);
      }
